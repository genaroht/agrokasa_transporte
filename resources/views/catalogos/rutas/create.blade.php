...
@section('header', 'Crear ruta de transporte')
...
<div class="bg-white rounded-lg shadow p-4 text-sm space-y-4">
    <form method="POST" action="{{ route('catalogos.rutas.store') }}">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-3 mb-4">
            ...
            <div class="flex items-center gap-2 mt-1">
                <input type="checkbox" name="activo" value="1" id="activo"
                       ...
                       @checked(old('activo', $ruta->activo ?? true))>
        ...
        <div class="space-y-2">
            ...
            <p class="text-[11px] text-gray-500">
                Cada fila representa un <strong>Lote o Red</strong> de la ruta y sus
                <strong>comedores asociados</strong>.  
                Escribe los comedores separados por coma
                (ejemplo: <em>Comedor 1, Comedor 2, Comedor 3</em>).
            </p>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Cargar old() si hubo error de validación
        if (Array.isArray(lotesOld) && lotesOld.length > 0) {
            lotesOld.forEach(function (l) {
                agregarFila(l);
            });
        } else {
